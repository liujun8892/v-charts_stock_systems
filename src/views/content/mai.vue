...
<script>
    import tpl from '@/components/index/mai'
  ...
  created(){
    ...
    methods:{
        setRemark(index,scope){
          let list = localStorage.getItem('codeArr3') ? JSON.parse(localStorage.getItem('codeArr3')):[];
          list[index].dateRemark = scope.dateRemark;
          localStorage.setItem('codeArr3',JSON.stringify(list));
        ...
        handleDelete(index,scope){
            this.$confirm('是否删除此条股票?', '提示', {
                ...
                let arr = JSON.parse(localStorage.getItem('codeArr3'));
                let idx = -1;
                ...
                // console.log(newArr,arr)
                localStorage.setItem('codeArr3',JSON.stringify(arr))
            this.$message({
                type: 'success',
                message: '删除成功!'
            });
        ...
        async getList(){
            ...
            this.list = localStorage.getItem('codeArr3') ? JSON.parse(localStorage.getItem('codeArr3')):[];
