        ...
        <el-table
            ...
            >
                <template slot-scope="scope">
                    ...
                    <span :class="{'red':scope.row.diff_money<0}"
                          v-if='scope.row.diff_money<0'>{{scope.row.diff_money}}</span>
                </template>
            </el-table-column>
            <el-table-column
                    prop="diff_rate"
            ...
            >
                <template slot-scope="scope">
                    ...
                    <span :class="{'red':scope.row.diff_rate<0}"
                          v-if='scope.row.diff_rate<0'>{{scope.row.diff_rate}}</span>
                </template>
            </el-table-column>
            <!-- <el-table-column
              prop="date"
            ...
            >
                <template slot-scope="scope">
                    <el-input v-model="scope.row.dateRemark" placeholder="请输入内容"
                              @input="setRemark(scope.$index,scope.row)"></el-input>
            ...
            >
                <template slot-scope="scope">
                    <el-input v-model="scope.row.pattern" placeholder="请输入内容"
                              @input="setPattern(scope.$index,scope.row)"></el-input>
                </template>
            </el-table-column>
            <el-table-column
                    prop="date"
            ...
            >
                <template slot-scope="scope">
                    <el-button
                            size="mini"
                            type="primary"
                            @click="checkTpl(scope.$index, scope.row)">查看k线图
                    </el-button>
                    <el-button
                            size="mini"
                            type="danger"
                    ...
                    <el-button
                            ...
                            type="success"
                            plain
                            @click="goPersonalStock(scope.$index, scope.row)"
                          >画线
                    </el-button>
                </template>
            </el-table-column>
...
<script>
    import tpl from '@/components/index/index'
    ...
    export default {
        ...
        methods: {
            ...
            goPersonalStock(index,scope){
                this.$router.push('/personal_stock?code='+scope.code)
                console.log('~~~'+index)
                console.log(scope)
            },
            setRemark(index, scope) {
                let list = localStorage.getItem('codeArr') ? JSON.parse(localStorage.getItem('codeArr')) : [];
                list[index].dateRemark = scope.dateRemark;
                localStorage.setItem('codeArr', JSON.stringify(list));
            ...
            checkTpl(index, scope) {
                ...
                this.tplShow = true;
                this.$refs.charts.init(scope.code, scope.name, 3);
            },
            handleDelete(index, scope) {
                this.$confirm('是否删除此条股票?', '提示', {
                    ...
                    let arr = JSON.parse(localStorage.getItem('codeArr'));
                    let idx = -1;
            ...
            async getList() {
                ...
                this.list = localStorage.getItem('codeArr') ? JSON.parse(localStorage.getItem('codeArr')) : [];
